            ...
            <ScrollView contentContainerStyle={styles.container}>
                {tasks.length === 0 ? (
                    ...
                    tasks.map((task: any, idx: number) => {
                        // Determine running/finished from data, but prefer the single `activeTaskIndex`
                        const hasStart = !!task.startTime;
                        const hasEnd = !!task.endTime;
                        const isFinished = hasStart && hasEnd;
                        // Only treat the task as currently running if its index matches the activeTaskIndex
                        ...
                        return (
                            <View key={task._id ?? idx} style={[styles.card, {backgroundColor: bg}]}>
                                <View style={{
                                    ...
                                    <Text
                                        style={[styles.taskName, {textAlign: isRTL ? 'right' : 'left'}]}>{task.name || t('untitledTask')}</Text>
                                    <View style={{flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center'}}>
                                        <Text
                                            style={[styles.meta, {textAlign: isRTL ? 'right' : 'left'}]}>{task.duration ? `${task.duration}m` : ''}</Text>
                                        {status === 'success' ? (
                                ...
                                {(task.startAtString ?? task.startAt) ? (
                                    <View style={[{
                                        ...
                                        <MaterialIcons name="access-time" size={16} color={textColor}
                                                       style={{marginHorizontal: isRTL ? 0 : 4}}/>
                                        <Text
                                            style={[styles.meta, {textAlign: isRTL ? 'right' : 'left'}]}>{formatStartAt(task.startAtString ?? task.startAt)}</Text>
                                    </View>
                                ) : null}
                                ...
                                {runningNow ? (
                                    ...
                                    <Text
                                        style={[styles.meta, {textAlign: isRTL ? 'right' : 'left'}]}>{`${t('elapsed')}: `}{isFinished ? formatElapsed(task.startTime, task.endTime) : (runningNow ? formatElapsed(task.startTime) : '00:00:00')}</Text>
                                )}
                                ...
                                <View style={{
                                    ...
                                    {showStart ? (
                                        ...
                                        <TouchableOpacity style={[styles.actionBtn, {backgroundColor: dangerBg}]}
                                                          onPress={() => handleEnd(idx)}>
