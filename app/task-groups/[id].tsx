...
export default function EditTaskGroupScreen() {
    ...
    const handleSubmit = async (data: Partial<ITaskGroup>) => {
        ...
        try {
            ...
            // Update local state with the saved data to prevent unnecessary re-sync
            setTaskGroup(prev => prev ? {...prev, ...data} : null);
    ...
    if (isLoading) {
        return (
            <ScreenTemplate title={taskGroup?.name || t('taskGroup') || 'Task Group'}>
    ...
    if (!taskGroup) {
        return (
            <ScreenTemplate title={t('taskGroup') || 'Task Group'}>
            <View style={styles.container}>
